...
{
    public static bool IsSendSMSEnable()
    {
        if (ConfigurationManager.AppSettings["SendSMSEnable"] != null && ConfigurationManager.AppSettings["SendSMSEnable"] == "Yes")
        {
            return true;
        }
        else
    ...
    public static string sendOTPtoMobile(string msg, string MobileNo,string template_id, out string SMSUserName)
    {
        string username = "";
        try
        {
            ...
            {
                //SMSUtility obj_SMSUtility = new SMSUtility();
                //obj_SMSUtility.Check_SSL_Certificate();
